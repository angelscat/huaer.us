...
if($action=='fbGood')
{
    ...
    echo "<a href=\"javascript:;\" onclick=\"postCommentAttitude('fbGood',{$aid})\">支持</a><em>[{$row['good']}]</em>";
    exit();
}
else if($action=='fbBad')
{
    AjaxHead();
    $fid = intval($fid);
    ...
    echo "<a href=\"javascript:;\" onclick=\"postCommentAttitude('fbBad',{$aid})\">反对</a><em>[{$row['bad']}]</em>";
    exit();
}
//查看评论
/*
